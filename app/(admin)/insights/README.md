...
2つの軸を明確に分けて表示する（C・体感ズレ対策）。

- **リーチ（views 合計）= どれだけ見られたか**。テーマ別・アカウント別の views 合計を棒グラフで表示。
  規模・到達の指標。「一番読まれたのは何か」はこちらで見る。
- **刺さり度（エンゲージメント率）= 見た人がどれだけ反応したか** = `(likes + replies + reposts + quotes + saves) / views`。
  `views=0` は率 = null（「—」表示）。パターン別/テーマ別（Threads のみ）・時間帯別（JST・両PF）。
