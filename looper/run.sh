...
# macOS: GNU coreutils の timeout を優先的に使用
    ...
    while true; do
        ...
        for id in "${batch[@]}"; do
            if git rev-list --count "HEAD..worktree/$id" 2>/dev/null | grep -qv '^0$'; then
                if git merge "worktree/$id" --no-ff -m "merge: $id" 2>/dev/null; then
                    log "Merged: $id"
                    merged_ids+=("$id")
                else
                    log "Merge失敗: $id（スキップ）"
            ...
            git worktree remove "$WT_BASE/$id" --force 2>/dev/null || true
            git branch -D "worktree/$id" 2>/dev/null || true
        done
        [ ${#merged_ids[@]} -eq 0 ] && { log "Verifier: マージ対象なし（スキップ）"; continue; }

        # verification 実行
        verification_cmd=$(jq -r --argjson m "$milestone" '.milestones[]|select(.milestone==$m)|.verification' "$PLAN")
        log "Verifier: $verification_cmd"
        if eval "$verification_cmd" >> "$LOG_DIR/verify-milestone${milestone}-w${wave}.log" 2>&1; then
            log "Verifier: verification 通過"
            # タスクを done: true に更新
            for id in "${merged_ids[@]+"${merged_ids[@]}"}"; do
                tmp=$(mktemp)
                jq --argjson m "$milestone" --arg id "$id" \
                    '(.milestones[] | select(.milestone==$m) | .tasks[] | select(.id==$id) | .done) |= true' \
                    "$PLAN" > "$tmp" && mv "$tmp" "$PLAN"
                log "Task done: $id"
            done
        else
            log "Verifier: verification 失敗（タスクは未完了のまま）"
        fi
    done
    
    # 全タスクが done なら Milestone を完了にする
    remaining=$(jq --argjson m "$milestone" '[.milestones[]|select(.milestone==$m)|.tasks[]|select(.done==false)]|length' "$PLAN")
    if [ "$remaining" -eq 0 ]; then
        tmp=$(mktemp)
        jq --argjson m "$milestone" '(.milestones[] | select(.milestone==$m) | .done) |= true' "$PLAN" > "$tmp" && mv "$tmp" "$PLAN"
        log "========== Milestone $milestone 完了 =========="
    else
        die "Milestone $milestone が完了しませんでした（残タスク: $remaining）。ログを確認: $LOG_DIR"
    fi
done
