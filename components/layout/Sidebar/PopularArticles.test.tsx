...
describe("PopularArticles", () => {
  describe("rendering", () => {
    it("renders the title", () => {
      render(<PopularArticles articles={mockArticles} />);
    ...
    it("renders empty state when no articles", () => {
      render(<PopularArticles articles={[]} />);
      expect(screen.getByTestId("popular-articles")).toBeInTheDocument();
  ...
  describe("limit", () => {
    it("limits the number of displayed articles", () => {
      ...
      render(<PopularArticles articles={manyArticles} limit={3} />);
      const links = screen.getAllByRole("link");
      expect(links).toHaveLength(3);
    });

    it("displays all articles when limit exceeds count", () => {
      render(<PopularArticles articles={mockArticles} limit={10} />);
      const links = screen.getAllByRole("link");
      expect(links).toHaveLength(3);
    });
  });
